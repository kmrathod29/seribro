  ...
  try {
    ...
    if (!req.user || req.user.role !== "company")
      return sendResponse(
        ...
        403,
        false,
        "Only companies can create payment orders"
      );

    const companyProfile = await CompanyProfile.findOne({ user: req.user._id });
    if (
      !companyProfile ||
      companyProfile._id.toString() !== project.companyId?.toString()
    )
      return sendResponse(res, 403, false, "You do not own this project");
    ...
    if (!project.selectedApplicationId) {
      return sendResponse(
        ...
        400,
        false,
        "No student/application selected for this project"
      );
    }
    ...
    try {
      order = await createRazorpayOrder(
        totalAmount,
        project._id,
        application.studentId || project.assignedStudent
      );
    } catch (err) {
      ...
      const razorpayErrorDescription =
        err && err.error && (err.error.description || err.error.reason)
          ...
          : err.message || "Unknown Razorpay error";
      const isRazorpayLimit = /limit|exceed|payout|daily|monthly|quota/i.test(
        String(razorpayErrorDescription)
      );
      ...
      const clientMessage = isRazorpayLimit
        ? "Payment creation failed due to Razorpay account limits. Please contact support or use an alternate payment method."
        : "Payment record created, but Razorpay is not configured. Complete configuration to generate an order.";
...
exports.verifyPayment = async (req, res) => {
  try {
    const { razorpayOrderId, razorpayPaymentId, razorpaySignature, projectId } =
      req.body;
    ...
    try {
      ...
      return sendResponse(
        ...
        400,
        false,
        "Razorpay verification failed: " + err.message
      );
    }
    if (!valid) {
    ...
    if (payment.status === "captured" || payment.status === "released") {
      ...
      return sendResponse(res, 200, true, "Payment already captured", {
        payment,
        project: projectExisting,
        flags: {
          hidePayNow: true,
          projectStatus: projectExisting?.status || "live",
        },
      });
    }
    ...
    if (studentProfile) {
      ...
      await sendNotification(
        studentProfile.user,
        "student",
        `Payment received for project ${project ? project.title : ""}`,
        "payment_received",
        "project",
        project?._id
      );
    ...
    try {
      ...
      if (companyProfile) {
        await sendNotification(
          companyProfile.user,
          "company",
          `Payment of ₹${payment.amount} received for project ${
    ...
    if (
      companyUser &&
      companyUser.email &&
      process.env.EMAIL_NOTIFY_ON_PAYMENT !== "false"
    ) {
      try {
        await sendEmail({
          email: companyUser.email,
          subject: "Payment successful",
          message: `<p>Payment of ₹${payment.amount} received for project ${
            project ? project.title : ""
          }</p>`,
        });
      } catch (e) {
        console.warn("Email send failed for verifyPayment:", e.message);
      }
    }
    ...
    try {
      await sendAdminNotification(
        `Payment captured for project ${project ? project.title : ""}`,
        "payment_captured",
        "project",
        project?._id
      );
    ...
    return sendResponse(res, 200, true, "Payment verified successfully", {
      payment,
      project,
      flags: { hidePayNow: true, projectStatus: "live" },
    });
  } catch (error) {
...
exports.markReadyForRelease = async (req, res) => {
  try {
    ...
    if (project.status !== "approved")
      return sendResponse(res, 400, false, "Project not approved yet");
    ...
    // Notify admins
    await sendAdminNotification(
      `Payment release pending for project ${project.title}`,
      "payment_release_pending",
      "project",
      project._id
    );

    return sendResponse(res, 200, true, "Payment marked ready for release", {
      paymentId: payment._id,
    });
  } catch (error) {
...
exports.getPendingReleases = async (req, res) => {
  try {
    ...
    if (Object.keys(searchFilter).length > 0) {
      ...
      total = await Payment.collection
...
exports.releasePayment = async (req, res) => {
  try {
    ...
    if (payment.status !== "ready_for_release")
      return sendResponse(res, 400, false, "Payment not ready for release");
    ...
    if (studentProfile) {
      ...
      await sendNotification(
        studentProfile.user,
        "student",
        `Payment of ₹${netAmount} released for project ${
          project ? project.title : ""
        }`,
        "payment_released",
        "project",
      ...
      try {
        ...
        if (
          ...
          await sendEmail({
            ...
            message: `<p>Payment of ₹${netAmount} has been released to you for project <strong>${
              project ? project.title : ""
            }</strong>.</p>`,
          });
        }
      } catch (e) {
        console.warn("Email send failed for releasePayment:", e.message);
      }
    }
    ...
    return sendResponse(res, 200, true, "Payment released successfully", {
      payment,
    });
  } catch (error) {
...
exports.refundPayment = async (req, res) => {
  try {
    ...
    if (!reason || reason.length < 5)
      return sendResponse(res, 400, false, "Refund reason required");
    ...
    const studentProfile = await StudentProfile.findById(payment.student);
    if (studentProfile) {
      studentProfile.earnings.pendingPayments = Math.max(
        0,
        (studentProfile.earnings.pendingPayments || 0) - refundAmount
      );
    ...
    if (
      ...
      try {
        await sendEmail({
          ...
          message: `<p>Refund of ₹${refundAmount} processed for project ${
            project ? project.title : ""
          }. Reason: ${reason}</p>`,
        });
      } catch (e) {
        console.warn("Email send failed for refundPayment:", e.message);
      }
    }
...
exports.getPaymentDetails = async (req, res) => {
  try {
    ...
    if (
      ...
      payment.student._id.toString() !== studentProfile._id.toString()
    ) {
  ...
  try {
    ...
    if (
      ...
      payment.student.toString() !==
        (await StudentProfile.findOne({ user: req.user._id }))._id.toString()
    )
      return sendResponse(res, 403, false, "Access denied");
    if (
      req.user.role === "company" &&
      payment.company.toString() !==
        (await CompanyProfile.findOne({ user: req.user._id }))._id.toString()
    )
      return sendResponse(res, 403, false, "Access denied");
...
exports.getStudentEarnings = async (req, res) => {
  try {
    ...
    const availableForWithdrawal = allPayments.reduce(
    ...
    for (const paymentId of paymentIds) {
      try {
        ...
        if (payment.status !== "ready_for_release") {
          ...
          results.errors.push({
            paymentId,
            error: "Payment not ready for release",
          });
          continue;
        }
        ...
        if (studentProfile) {
          ...
          await sendNotification(
            studentProfile.user,
            "student",
            `Payment of ₹${payment.amount} released for project ${
              project ? project.title : ""
            }`,
            "payment_released",
            "project",
          ...
          try {
            ...
            if (
              ...
              await sendEmail({
                email: studentUser.email,
                subject: "Payment released",
                message: `<p>Payment of ₹${
                  payment.amount
                } has been released to you for project <strong>${
                  project ? project.title : ""
                }</strong>.</p>`,
              });
            }
