...
const initRazorpay = () => {
  if (!process.env.RAZORPAY_KEY_ID || !process.env.RAZORPAY_KEY_SECRET) {
    throw new Error('Razorpay keys missing in environment. Set RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET');
  }
  if (!Razorpay) {
    throw new Error('Razorpay SDK not installed. Run `npm i razorpay` in backend');
  }
