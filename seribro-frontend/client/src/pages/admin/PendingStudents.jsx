      ...
      <div className="space-y-6 pt-20">
        ...
        <div>
          <h1 className="text-4xl font-black text-amber-500 mb-2">
            Pending <span className="text-primary">Students</span>
          </h1>
          <p className="text-gray-600">Review and approve student profile submissions</p>
        </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-2xl p-6 border border-blue-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-black text-blue-600">{list.length}</p>
            </div>
            <Users className="text-blue-300 opacity-50" size={48} />
          </div>
        </div>
                ...
                {list.length ? list.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold text-navy">{s.profileCompletion}%</span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {actionId === s.id ? <Loader size={16} className="animate-spin" /> : <CheckCircle size={16} />}
                          Approve
                        </button>
                        <button
                          onClick={() => reject(s.id)}
                        ...
                        >
                          {actionId === s.id ? <Loader size={16} className="animate-spin" /> : <XCircle size={16} />}
                          Reject
                        </button>
                      </div>
                    </td>
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-12 text-center">
                      ...
                      <p className="text-gray-500 font-medium">No pending students for review</p>
                    </td>
                  </tr>
                )}
              </tbody>
